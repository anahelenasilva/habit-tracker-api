...
@Injectable()
export class FailHabitUseCase {
  constructor(
    private readonly habitRepository: HabitRepository,
    private readonly habitLogRepository: HabitLogRepository,
  ) { }

  async execute(input: FailHabitUseCase.Input): Promise<FailHabitUseCase.Output> {
...
export namespace FailHabitUseCase {
  export type Input = {
    habitId: string;
    accountId: string;
